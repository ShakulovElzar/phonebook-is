...
const Register = () => {
	...
	useEffect(
		() => {
			axios.get('http://10.200.24.103:8089/department/').then(response => setDepartments(response.data));
			axios.get('http://10.200.24.103:8089/jobtitle/').then(t => setJobTitles(t.data));
		},
		[]
	);
		...
		<div className="page-body">
			...
			{changeContent
				...
				: <form className="form" onSubmit={registerForm}>
							...
							<div className="register-input">
								...
								<FormControl>
									...
									<Select sx={{ width: 350 }} onChange={t => setDepartment(t.target.value)} label="Выбрать департамент">
										{departments.map((item, index) => <MenuItem key={index} value={item.id}>{item.title}</MenuItem>)}
									</Select>
								</FormControl>
							</div>
							<div className="register-input">
								...
								<FormControl>
									...
									>
										{jobtitles.map((item, index) => <MenuItem key={index} value={item.id}>{item.jobtitle}</MenuItem>)}
									</Select>
								</FormControl>
							</div>
							<div className="register-input">
						...
						<Button type="submit" variant="outlined" style={{ marginLeft: 0, marginTop: 20 }}>Регистрация</Button>
					</form>}
